...
public abstract class AbstractSubscriberWorker implements Runnable {
	...
	@Override
	public void run() {
		logger.info(BaseUtils.getLogText("Eventbus subscribe 【%s】 topic 【%s】 is begining!", name,
				ISubPubProvider.PUBSUB_CHANNEL));
		if (isSubLoop) {
			while (true) {
				try {
					...
					logger.error(BaseUtils.getLogText("Eventbus subscribe 【%s】 topic 【%s】 error!", name,
				...
				logger.error(BaseUtils.getLogText("Eventbus subscribe 【%s】 topic 【%s】 is fail!Has retry【%s】counts!",
						name, Constants.CORAL_EVENTBUS_CHANNEL, RESUBCOUNT.intValue()));
			}
		} else {
			runInternal();
		}
