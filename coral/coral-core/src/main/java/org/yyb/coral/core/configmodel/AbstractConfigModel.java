	...
	protected void appendConfigParams(Map<String, String> parameters, String prefix) {
		...
		for (Method method : methods) {
			try {
				...
				if (isConfigMethod(method)) {
					...
					if (value == null || StringUtils.isBlank(String.valueOf(value))) {
						if (configDesc != null && configDesc.required()) {
							throw new ConfigModelRuntimeException(BaseUtils.getLogText(
									"%s.%s should not be null or empty", this.getClass().getSimpleName(), key));
						}
						continue;
					}
				...
				throw new ConfigModelRuntimeException(BaseUtils.getLogText("Error when append params for config: %s.%s",
	...
	public String toString() {
		try {
			...
			for (Method method : methods) {
				try {
					String name = method.getName();
					isMatch = (name.startsWith("get") || name.startsWith("is")) && !"getClass".equals(name)
