...
public class RedisUtis {
     ...
     * @return
     */
    public static <T> RedisInvokeResult<T> runWithJedis(RedisInvokeHander<T> redisInvokeHander, String providerName, String configGroup) {
        ...
        try {
            if (jedisProvider != null) {
                ...
                if (jedis != null) {
                    ...
                    logger.warn(
                            BaseUtils.getLogText("get jedis is null,please init jedis!providerName=%s,configGroup=%s", providerName, configGroup));
                }
            } else {
                result.setResultCode(RedisInvokeResultEnum.NOREDISPROVIDER);
                logger.warn(
                        BaseUtils
                                .getLogText("get JedisProvider is null,please init jedis!providerName=%s,configGroup=%s", providerName, configGroup));
            }
        } catch (Exception e) {
            result.setResultCode(RedisInvokeResultEnum.ERROR);
            logger.error(BaseUtils.getLogText("operate jedisClient error!!providerName=%s,configGroup=%s", providerName, configGroup), e);
        } finally {
            if (jedis != null) {
                jedis.close();
            }
     ...
     * @return
     */
    public static RedisInvokeResult<Void> runWithJedis(final RedisInvokeVoidHander<Void> redisInvokeVoidHander, String providerName,
            String configGroup) {
     ...
     * @return
     */
    public static boolean tryLock(String lockKey, long leaseTime, boolean releaseLock, IRedisLockHander redisLockHander) {
        return tryLock(Constants.CORAL_DEFAULT, Constants.CORAL_DEFAULT, lockKey, leaseTime, releaseLock, redisLockHander);
    }
     ...
     * @return
     */
    public static boolean tryLock(String providerName, String configGroup, String lockKey, long leaseTime, boolean releaseLock,
                ...
                try {
                    ...
                    logger.error(
                            BaseUtils.getLogText(
                                    "Reids tryLock is error!providerName=%s,configGroup=%s,lockKey=%s",
                                    providerName,
                                    configGroup,
                                    lockKey),
                            e);
