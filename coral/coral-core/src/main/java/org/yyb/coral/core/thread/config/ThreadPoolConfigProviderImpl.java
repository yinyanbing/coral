...
public class ThreadPoolConfigProviderImpl extends AbstractConfigModelProvider {
    ...
    protected IConfigModel makeConfigModel(String configGroup) {
        ...
        configModel.setMaximumPoolSize(NumberUtils.toInt(maximumPoolSizeStr, ThreadPoolConfigEnum.MAXINUMPOOLSIZE.defaultValue));

        String keepAliveTimeStr = getConfigValue(configGroup, KEEPALIVETIME_KEY);
        configModel.setKeepAliveTime(
                NumberUtils.toLong(keepAliveTimeStr, NumberUtils.toLong(String.valueOf(ThreadPoolConfigEnum.KEEPALIVETIME.defaultValue))));

        String workQueueSizeStr = getConfigValue(configGroup, WORKQUEUESIZE_KEY);
        configModel.setWorkQueueSize(NumberUtils.toInt(workQueueSizeStr, ThreadPoolConfigEnum.WORKQUEUESIZE.defaultValue));
        return configModel;
    }
