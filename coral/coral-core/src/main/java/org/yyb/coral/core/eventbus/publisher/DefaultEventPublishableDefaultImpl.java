...
public class DefaultEventPublishableDefaultImpl implements IEventPublishable {
	...
	public boolean publish(IApplicationEvent event) {
		...
		if (event instanceof LocalApplicationEventImpl || AppIdentificationEventEnum.LOCAL == AppIdentificationEventEnum
				.getInitType(event.getEventIdentification())) {
		...
		if (event instanceof RemoteApplicationEventImpl
				|| AppIdentificationEventEnum.REMOTE == AppIdentificationEventEnum
						.getInitType(event.getEventIdentification())) {
			CompomentInfo compomentInfo = CoralApplication
					.getCompomentInfo(CoralCompomentsEnum.EVENTBUS_REMOTE.getName());
			if (compomentInfo != null) {
				IRemoteSubPubProvider remoteSubPubProvider = ExtensionLoader
						.getExtensionLoader(IRemoteSubPubProvider.class).getExtension(compomentInfo.getProviderName());
