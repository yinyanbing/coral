...
public class RedissonProviderDefaultImpl extends AbstractIntegrateClientProvider<RedissonClient>
	...
	protected Object doMakeClientPoolInternal(IConfigModel configModel) {
		if (configModel instanceof RedisConfigModel) {
			...
			RedissonInitTypeEnum initTypeEnum = RedissonInitTypeEnum
					.getInitType(redisConfigModel.getRedissonInitType());
	...
	private RedissonClient getSingleton(RedisConfigModel configModel) {
		...
		SingleServerConfig singleServerConfig = config.useSingleServer().setAddress(address)
				.setPassword(configModel.getPassWord());
